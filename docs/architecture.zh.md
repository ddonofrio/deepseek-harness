...
当前步骤输入和路由请求头持久化后，循环会在发送冻结请求前让已配置的压缩服务再执行一次压力检查。如果替换成功，循环会从会话日志重新构建请求；如果启发式压力没有预测到提供商的限制，则由提供商确认的上下文溢出继续作为后备路径。
...
| 拦截请求、工具或轮次 | 使用相应的 `agent/*` 或 `tools/*` 事件；`agent/turn-stopping` 观察终止原因并可引导另一个步骤 |
