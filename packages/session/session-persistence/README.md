...
The persisted unit IS the existing `SessionEvent` (event-sourced model — the log is the single source of truth), so there is no parallel "persisted message" type. Metadata that is NOT replayable conversation state (format version, cwd, lineage, seed boundary, origin, delegation depth, ephemeral status) travels separately as `SessionHeader`, owned by `dsh-session` and re-exported here. Ephemeral sessions are intentionally excluded from persistence and do not create backend artifacts.
