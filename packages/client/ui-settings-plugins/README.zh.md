...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）以及 DeepSeek 搜索提供方（`web-search-deepseek`）。agent-loop 插件还会把 LLM 循环恢复偏好贡献到 General 设置中，紧跟在忙碌时 Enter 的 Composer 偏好之后；它编辑同一个 `agent-loop` 命名空间，并应用于新 agent，除非显式的 agent 选项覆盖它。
