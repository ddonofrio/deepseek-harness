...
The cards this package ships cover the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and the DeepSeek search provider (`web-search-deepseek`). The agent-loop plugin also contributes the LLM loop recovery preference to General settings, immediately after the busy-Enter Composer preference; it edits the same `agent-loop` namespace and applies to new agents unless an explicit agent option overrides it.
