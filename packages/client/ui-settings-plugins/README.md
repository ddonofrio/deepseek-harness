...
The agent-loop plugin also contributes the LLM loop recovery preference to General settings, immediately after the busy-Enter Composer preference; it edits the same `agent-loop` namespace and applies to new agents unless an explicit agent option overrides it.
