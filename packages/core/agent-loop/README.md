...
With `compactBeforeFailing`, the fourth detection closes the failed turn, waits for the driver to become idle, and calls the installed `ctx.compaction` provider's `compactNow` path. Waking input remains queued while that standalone maintenance operation runs. A successful replacement puts the original turn input at the front of `next-turn`, so it is derived in a fresh turn; a missing useful range or backend failure remains terminal.
