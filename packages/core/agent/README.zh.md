...
`AgentOptions` 提供初始的提供方／模型路由、可选的正数 `maxTokens` 输出上限，以及可选的 `loopDetection` 策略。具体循环会解析确切模型的适配器默认值，把生效上限记录到请求 header，并应用到每次对话模型请求；显式 Agent 选项优先，省略时由适配器或提供方路由默认值控制。除非 `enabled` 为 true，否则循环检测不会启用；提示词和 `includeLoop` 选择由接收该选项的 agent 所有。
